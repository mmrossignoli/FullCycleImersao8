...
export const Mapping: FunctionComponent = () => {
  ...
  useEffect(() => {
    socketIORef.on("connect", () => console.log("conectou"));
    console.log(socketIORef.listenersAnyOutgoing());
    const handler = (data: {
      routeId: string;
      ...
      if (data.finished) {
        finishRoute(route);
        socketIORef?.off("new-position", handler);
      }
    };
    socketIORef?.on("new-position", handler);
    return () => {
      /* socketIORef?.off("new-position", handler); */
    };
  ...
  const startRoute = useCallback(
    (event: FormEvent) => {
      ...
      try {
        ...
        fetch(`${API_URL}/routes/start`, {
          method: "POST",
          body: JSON.stringify({
            routeId: routeIdSelected,
            clientId: socketIORef.id,
          }),
